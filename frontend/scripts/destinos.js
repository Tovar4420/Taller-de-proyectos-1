...
function renderDestinos(lista) {
  ...
  // SIN cargar ML aquí — se carga solo al abrir el modal de detalle
}

// ── Predicción ML (se llama solo desde el modal de detalle) ──────
async function cargarPrediccionML(destinoId) {
  const body = document.getElementById('detalle-ml-body');
  if (!body) return;
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/prediccion/${destinoId}?dia=${dia}&mes=${mes}`);
    if (!res.ok) throw new Error('Error predicción');
    const data = await res.json();

    const iconoEstado = {
      bajo: '🟢', moderado: '🟡', alto: '🟠', critico: '🔴'
    }[data.estado_predicho] || '⚪';

    body.innerHTML = `
      <div class="ml-prediccion">
        <div class="ml-row">
          <span class="ml-label">Afluencia estimada hoy</span>
          <span class="ml-valor" style="color:${data.color_estado}">
            ${iconoEstado} ${data.porcentaje_predicho}%
          </span>
        </div>
        <div class="ml-barra-fondo">
          <div class="ml-barra-relleno" style="width:${data.porcentaje_predicho}%;background:${data.color_estado}"></div>
        </div>
        <p class="ml-recomendacion-txt">${data.recomendacion}</p>
        <div class="ml-metricas">
          <span title="Coeficiente de determinación R²">
            📊 Exactitud: <strong>${data.metricas_modelo.exactitud_r2}%</strong>
          </span>
          <span title="Precisión del modelo">
            🎯 Precisión: <strong>${data.metricas_modelo.precision_pct || data.metricas_modelo.precision}%</strong>
          </span>
        </div>
      </div>
    `;
  } catch (err) {
    body.innerHTML = `<p class="ml-error">Predicción no disponible</p>`;
  }
}

// ── ML Caso 2: Ranking mejores días para visitar ────────────────
async function cargarMejoresDiasML(destinoId) {
  const body = document.getElementById('detalle-ml2-body');
  if (!body) return;
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/prediccion/${destinoId}/mejores-dias?mes=${mes}`);
    ...
    const filas = data.ranking.map((r, i) => `
      <tr class="ml2-fila ${i === 0 ? 'ml2-mejor' : i === data.ranking.length - 1 ? 'ml2-peor' : ''}">
        <td class="ml2-pos">${i + 1}</td>
        <td class="ml2-dia">
          ${r.icono} ${r.dia}
          ${r.etiqueta ? `<span class="ml2-etiqueta">${r.etiqueta}</span>` : ''}
        </td>
        <td class="ml2-pct">
          <div class="ml2-barra-wrap">
            <div class="ml2-barra" style="width:${r.ocupacion_predicha}%;background:${_colorPorRecomendacion(r.recomendacion)}"></div>
          </div>
          <span class="ml2-num">${r.ocupacion_predicha}%</span>
        </td>
        <td><span class="ml2-rec ml2-rec-${r.color}">${r.recomendacion}</span></td>
      </tr>
    `).join('');

    body.innerHTML = `
      <div class="ml2-resumen">
        <span>📅 ${MESES[data.mes]} · ${data.destino_nombre}</span>
        <span class="ml2-mejor-dia">✅ Mejor día: <strong>${data.resumen.mejor_dia}</strong></span>
      </div>
      <table class="ml2-tabla">
        <thead>
          <tr>
            <th>#</th>
            <th>Día</th>
      ...
      ${data.resumen.dias_evitar.length > 0
        ? `<p class="ml2-aviso">⚠️ Días a evitar: <strong>${data.resumen.dias_evitar.join(', ')}</strong></p>`
        : ''}
    `;
  } catch (err) {
    body.innerHTML = `<p class="ml-error">Ranking no disponible</p>`;
  }
}

function _colorPorRecomendacion(rec) {
  return { 'Ideal': '#4caf50', 'Bueno': '#8bc34a', 'Regular': '#ff9800', 'Evitar': '#f44336' }[rec] || '#999';
}
...
async function abrirDetalleDestino(id) {
  try {
    ...
    // Cargar ambos casos de uso ML al abrir el detalle
    cargarPrediccionML(d.id);
    cargarMejoresDiasML(d.id);
